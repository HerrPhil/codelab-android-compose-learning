...
fun ExploreSection(
    ...
    Surface(modifier = modifier.fillMaxSize(), color = Color.White, shape = BottomSheetShape) {
        Column(modifier = Modifier.padding(start = 24.dp, top = 20.dp, end = 24.dp)) {
            ...
            // Lesson Codelab: derivedStateOf step
            // Lesson: Show "Scroll to top" button when the first item of the list is not visible
            Box(Modifier.weight(1f)) {
                val listState = rememberLazyListState()
                ExploreList(exploreList, onItemClicked, listState = listState)

                // The scroll up button when necessary
                // Show the button if the first visible item is past
                // the first item. We use a remembered derived state to
                // minimize unnecessary composition
                val showButton by remember {
                    derivedStateOf {
                        Log.i("list", "first visible item index = ${listState.firstVisibleItemIndex}")
